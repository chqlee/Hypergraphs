...
def getSeedList(G,key_nodes_total):
    ...
    list_mean = []
    for k, v in path_len.items():
        v_list = list(v.values())
        if len(v_list) < 15233:
            a = np.array([15233]*(15233 - len(v_list)))
            v_list.extend(a)
        list_mean.append(np.mean(np.array(v_list)))
    ser1 = pd.Series(list_mean).rank(method='first', ascending=True)
    path_min_list = ser1[ser1.values <= key_nodes_total].index
    return path_min_list
...
if __name__ == '__main__':
    ...
    inf_spread_matrix = []
    seed_total_list = getSeedList(DG, seed_size)
    R = 200
    for r in range(0, R):
        inf_spread_list = []
        for i in range(1, seed_size + 1):
            DG = copy.deepcopy(DG)
            seed_list = seed_total_list[:i]
            print('-------------------- ROUND ' + str(r) + ' ----------------------')
            print('经 Distance 算法，您需寻找的' + str(i) + '个种子节点如下')
            print(seed_list)
    ...
    plt.ylim(-6, 163)
    plt.plot(x, y, marker='s', markersize=4., label='Distance', color='g')
    plt.legend()
    plt.show()
